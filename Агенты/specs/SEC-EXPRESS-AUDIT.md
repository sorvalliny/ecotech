...
**Описание:** Массив `DEFAULT_USERS` содержит реальное ФИО сотрудника и корпоративный email (`pmo@rwb.ru`). Репозиторий публичный (github.com/sorvalliny/ecotech) — данные видны любому. Это нарушение ФЗ-152 и внутренних политик ИБ.

**Конкретные PII:**
- Строка 14: `name: 'PMO Lead'`, `email: 'pmo@rwb.ru'`
...
```
Реальные email использовать только в localStorage при деплое, не хардкодить в коде. Также убрать `pmo@rwb.ru` из `brief.html` строка 461 (mailto-ссылка).
