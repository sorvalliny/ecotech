...
(function () {
  ...
  var ICONS = {
    ...
    'pen-line':     '<path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z"/>',
    ...
    'cpu':          '<rect x="9" y="9" width="6" height="6"/><path d="M3 9h2M3 12h2M3 15h2M19 9h2M19 12h2M19 15h2M9 3v2M12 3v2M15 3v2M9 19v2M12 19v2M15 19v2"/><rect x="5" y="5" width="14" height="14" rx="1"/>'
  };

  function injectSprite() {
    if (document.getElementById('ei-sprite')) return;
    ...
    Object.keys(ICONS).forEach(function (n) {
      defs += '<symbol id="ic-' + n + '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">' + ICONS[n] + '</symbol>';
    });
    defs += '</defs>';
    svg.innerHTML = defs;
   ...
   * @param {string} name  Icon ID (e.g. 'home', 'rocket')
   * @param {number} size  Width/height in px (default 16)
   * @param {string} cls   Extra CSS classes (e.g. 'ei-violet ei-20')
   * @returns {string} HTML string
   */
  function svg(name, size, cls) {
    size = size || 16;
    var c = 'ei' + (cls ? ' ' + cls : '');
    return '<svg class="' + c + '" width="' + size + '" height="' + size + '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><use href="#ic-' + name + '"/></svg>';
  }

  window.EI = { svg: svg, ICONS: ICONS };
